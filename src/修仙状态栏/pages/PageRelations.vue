        ...
        <div class="xy-wild-head">
          ...
          <div class="xy-wild-meta">
            ...
            <div class="xy-wild-line2">
              ...
              <span v-if="npc.五行" class="xy-element xy-element-mini" :style="{ '--el': elColor(npc.五行) }">{{
                npc.五行 === '混沌' ? '混' : npc.五行
              }}</span>
            </div>
          </div>
      ...
      >
        <div class="xy-npc-head">
          ...
          <div class="xy-npc-info">
            <div class="xy-npc-line1">
              <span class="xy-name-copy-row">
                ...
                <CopyNameButton :text="String(name)" label="人物名" />
              </span>
            ...
            <div class="xy-npc-line2">
              ...
              <span
                ...
                :class="{
                  ...
                  'xy-bool-off': state.editMode && npc.体质?.元阳 === false,
                  'xy-bool-null': state.editMode && npc.体质?.元阳 == null,
                }"
              ...
              <span
                ...
                :class="{
                  'xy-bool-toggle': state.editMode,
                  'xy-bool-off': state.editMode && npc.体质?.元阴 === false,
                  'xy-bool-null': state.editMode && npc.体质?.元阴 == null,
                }"
          ...
          >
            <svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor" aria-hidden="true">
              <path
                d="m15.5 2 .95 2.55L19 5.5l-2.55.95L15.5 9l-.95-2.55L12 5.5l2.55-.95L15.5 2ZM6.9 7.1l1.45 3.9 3.9 1.45-3.9 1.45-1.45 3.9L5.45 13.9l-3.9-1.45 3.9-1.45L6.9 7.1Zm10.47 5.4 1.02 2.73 2.73 1.02-2.73 1.02-1.02 2.73-1.02-2.73-2.73-1.02 2.73-1.02 1.02-2.73Z"
              />
            </svg>
          </button>
        </div>
        ...
        <div v-if="state.openedNPC === name" class="xy-npc-body" @click.stop>
          ...
          <div class="xy-npc-grid">
            <div class="xy-mini">
              ...
              <p>
                ...
                <small v-if="npc.寿元"
                  >貌 <EditableValue v-model.number="npc.寿元.外观年龄" type="number" label="外观年龄" :min="0"
                /></small>
              </p>
            </div>
            <div class="xy-mini">
              ...
              <p>
                <span
                  ><EditableValue v-if="npc.灵根" v-model="npc.灵根.名称" label="灵根名称" :format="v => v || '未检测'"
                /></span>
                <span
                  v-for="el in npc.灵根?.五行 || []"
                  :key="el"
            ...
            <div class="xy-mini">
              ...
              <p>
                ...
                <small v-if="npc.体质">
                  悟<EditableValue
                    v-model.number="npc.体质.悟性"
                    type="number"
                    label="悟性"
                    :min="0"
                  />/骨<EditableValue
                    v-model.number="npc.体质.根骨"
                    type="number"
                    label="根骨"
                    :min="0"
                  />/感<EditableValue v-model.number="npc.体质.气感" type="number" label="气感" :min="0" />
                </small>
              </p>
            </div>
            <div class="xy-mini">
              ...
              <p>
                ...
                <small v-if="npc.修炼进度">
                  <EditableValue
                    v-model.number="npc.修炼进度.当前进度"
                    type="number"
                    label="当前进度"
                    :min="0"
                  />/<EditableValue v-model.number="npc.修炼进度.进度上限" type="number" label="进度上限" :min="1" />
                </small>
              </p>
            </div>
          </div>
            ...
            <div class="xy-npc-bars">
              <div v-if="npc.资源池.气血" class="xy-unit-bar">
                ...
                <span class="xy-unit-bar-num">
                  <EditableValue
                    v-model.number="npc.资源池.气血.现值"
                    type="number"
                    label="气血现值"
                    :min="0"
                  />/<EditableValue v-model.number="npc.资源池.气血.上限" type="number" label="气血上限" :min="1" />
                </span>
              </div>
              <div v-if="npc.资源池.灵气" class="xy-unit-bar">
                ...
                <span class="xy-unit-bar-num">
                  <EditableValue
                    v-model.number="npc.资源池.灵气.现值"
                    type="number"
                    label="灵气现值"
                    :min="0"
                  />/<EditableValue v-model.number="npc.资源池.灵气.上限" type="number" label="灵气上限" :min="1" />
                </span>
              </div>
              ...
              >
                ...
                <div class="xy-art-head">
                  ...
                  <button
                    ...
                    @click.stop="
                      toggleNpcArt(name, aname as string, !isArtEffectivelyActive(npc, aname as string, art))
                    "
                  >
            ...
            <div class="xy-npc-storage">
              ...
              <div v-if="!_.isEmpty(npc.物品)" class="xy-npc-sub">
                ...
                <div v-show="isSectionOpen(name, '物品')" class="xy-item-grid xy-item-grid-mini xy-collapse-body">
                  <article v-for="(it, iname) in npc.物品" :key="iname" class="xy-item" :class="['xy-q-bg-' + it.品质]">
                    <button
                      type="button"
                      class="xy-trash"
                      title="删除此物品"
          ...
          <div v-if="!_.isEmpty(npc.性器) || state.editMode" class="xy-mini-block">
            <h4 class="xy-genital-toggle" @click="state.genitalOpen[String(name)] = !state.genitalOpen[String(name)]">
              性器
...
// 区分人物条目与无主战斗单位条目
const characterRelations = computed(() => sortedRelations.value.filter(({ npc }) => !npc?.类型 || npc.类型 === '人物'));
...
.xy-wild-type-tag {
  ...
  color: var(--xy-cinnabar-deep, #6e4f1d);
  border: 1px solid rgba(160, 127, 72, 0.3);
